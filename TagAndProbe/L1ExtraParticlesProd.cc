...
// $Id: L1ExtraParticlesProd.cc,v 1.1 2011/12/14 17:42:51 rybinska Exp $
//
//
...
L1ExtraParticlesProd::L1ExtraParticlesProd(const edm::ParameterSet& iConfig)
   : produceMuonParticles_( iConfig.getParameter< bool >(
	...
	"muonSource" ) ),
     //muonregSource_( iConfig.getParameter< edm::InputTag >(
	//"muonregSource" ) ),
...
void L1ExtraParticlesProd::produce( edm::Event& iEvent,const edm::EventSetup& iSetup){
	...
	auto_ptr< L1MuonParticleCollection > muColl(new L1MuonParticleCollection );
	if( produceMuonParticles_ ){
		...
		vector< L1MuRegionalCand > hwMuRegRPCBCands ;
		vector< L1MuRegionalCand > hwMuRegRPCFCands ;
		vector< L1MuRegionalCand > hwMuRegDTCands ;
		vector< L1MuRegionalCand > hwMuRegCSCCands ;
		if(!hwMuCollection.isValid()){
			LogDebug("L1ExtraParticlesProd")<< "\nWarning: L1MuGMTReadoutCollection with "<< muonSource_<< "\nrequested in configuration, but not found in the event."<<endl;
		}else{
			if( centralBxOnly_ ){
				hwMuCands = hwMuCollection->getRecord().getGMTCands() ;
				hwMuRegRPCBCands= hwMuCollection->getRecord().getBrlRPCCands() ;
				hwMuRegRPCFCands= hwMuCollection->getRecord().getFwdRPCCands() ;
				hwMuRegDTCands = hwMuCollection->getRecord().getDTBXCands();
				hwMuRegCSCCands = hwMuCollection->getRecord().getCSCCands() ;
			}else{
				...
				for( ; rItr != rEnd ; ++rItr ){
					...
					hwMuRegRPCBCands.insert( hwMuRegRPCBCands.end(),tmpRegBrlCands.begin(),tmpRegBrlCands.end() ) ;
					hwMuRegRPCFCands.insert( hwMuRegRPCFCands.end(),tmpRegFwdCands.begin(),tmpRegFwdCands.end() ) ;
					hwMuRegDTCands.insert( hwMuRegDTCands.end(),tmpRegDTCands.begin(),tmpRegDTCands.end() ) ;
					hwMuRegCSCCands.insert( hwMuRegCSCCands.end(),tmpRegCSCCands.begin(),tmpRegCSCCands.end() ) ;
				}
			}
			...
			vector< L1MuRegionalCand >::const_iterator muItrRegRPCB = hwMuRegRPCBCands.begin() ;
			vector< L1MuRegionalCand >::const_iterator muEndRegRPCB = hwMuRegRPCBCands.end() ;
			vector< L1MuRegionalCand >::const_iterator muItrRegRPCF = hwMuRegRPCFCands.begin() ;
			vector< L1MuRegionalCand >::const_iterator muEndRegRPCF = hwMuRegRPCFCands.end() ;
			vector< L1MuRegionalCand >::const_iterator THISmuItrRegRPC;
			vector< L1MuRegionalCand >::const_iterator muItrRegDT = hwMuRegDTCands.begin() ;
			vector< L1MuRegionalCand >::const_iterator muEndRegDT = hwMuRegDTCands.end() ;
			vector< L1MuRegionalCand >::const_iterator THISmuItrRegDT;
			vector< L1MuRegionalCand >::const_iterator muItrRegCSC = hwMuRegCSCCands.begin() ;
			vector< L1MuRegionalCand >::const_iterator muEndRegCSC = hwMuRegCSCCands.end() ;
			vector< L1MuRegionalCand >::const_iterator THISmuItrRegCSC;
			for( int i = 0 ; muItr != muEnd ; ++muItr, ++i ){
				if( !muItr->empty() && muItr->bx()==0){
					...
					//std::cout<<"L1Extra eta= "<< eta;
					double phi = muScales->getPhiScale()->getLowEdge( muItr->phiIndex() ) ;
					// std::cout<<"L1Extra phi= "<< phi<< std::endl;
					math::PtEtaPhiMLorentzVector p4( pt,eta,phi,muonMassGeV_ ) ;
						   
					if (muItr->quality()==5|| muItr->quality()==7){
		                if (muItr->isFwd()) THISmuItrRegRPC = hwMuRegRPCFCands.begin()+ muItr->getRPCIndex();
		                else THISmuItrRegRPC = hwMuRegRPCBCands.begin()+ muItr->getRPCIndex();
					}
					if ((muItr->quality()==6|| muItr->quality()==7)&&(muItr->detector() == 2 || muItr->detector() == 3))
		                THISmuItrRegDT = hwMuRegDTCands.begin()+ muItr->getDTCSCIndex();
					if((muItr->quality()==4 || muItr->quality()==6||muItr->quality()==7 )&& (muItr->detector() == 4 || muItr->detector() == 5 ))
						THISmuItrRegCSC = hwMuRegCSCCands.begin()+ muItr->getDTCSCIndex();
					/// 1=rpc, 2=dtbx, 4=csc, 3=rpc+dtbx, 5=rpc+csc
					if (muItr->quality()==5) //RPC only
						muColl->push_back(L1MuonParticle( muItr->charge(),p4,*muItr,*THISmuItrRegRPC,muItr->bx(),true,false,false));
					if (muItr->quality()==6 && muItr->detector()== 2) //DT only
						muColl->push_back(L1MuonParticle( muItr->charge(),p4,*muItr,*THISmuItrRegDT,muItr->bx(),false,true,false));
					if (muItr->quality()==6 && muItr->detector()== 4) //CSC only
						muColl->push_back(L1MuonParticle( muItr->charge(),p4,*muItr,*THISmuItrRegCSC,muItr->bx(),false,false,true));
					if (muItr->quality()==4) //CSC only, low quality
						muColl->push_back(L1MuonParticle( muItr->charge(),p4,*muItr,*THISmuItrRegCSC,muItr->bx(),false,false,true));
					if (muItr->quality()==7 && muItr->detector()== 3)//RPC+DT
						muColl->push_back(L1MuonParticle( muItr->charge(),p4,*muItr,*THISmuItrRegRPC,*THISmuItrRegDT,muItr->bx(),true,true,false));
					if (muItr->quality()==7 && muItr->detector()== 5) //RPC+CSC
						muColl->push_back(L1MuonParticle( muItr->charge(),p4,*muItr,*THISmuItrRegRPC,*THISmuItrRegCSC,muItr->bx(),true,false,true));
					if (muItr->quality()==2 || muItr->quality()==3) //No muon
						muColl->push_back(L1MuonParticle( muItr->charge(),p4,*muItr,muItr->bx(),false,false,false));
				}
			}
		}
	}
	OrphanHandle< L1MuonParticleCollection > muHandle = iEvent.put( muColl );
...
/*std::cout<< setiosflags(std::ios::fixed)
          << "||"<< std::setprecision(2)<< std::setw(9)<<std::right<< "L1Quality"
          << "||"<< std::setprecision(2)<< std::setw(9)<<std::right<< "L1 eta"
          << "||"<< std::setprecision(2)<< std::setw(9)<< std::right<<"RPC eta"
          << "||"<< std::setprecision(2)<< std::setw(9)<< std::right<<"RPC-L1eta"                   
          << "||"<< std::setprecision(2)<< std::setw(9)<<std::right<< "L1 phi"
          << "||"<< std::setprecision(2)<< std::setw(9)<< std::right<<"RPC phi"
          << "||"<< std::setprecision(2)<< std::setw(9)<< std::right<<"RPC-L1phi"      
           << "||"<< std::setprecision(2)<< std::setw(9)<<std::right<< "L1 pt"
          << "||"<< std::setprecision(2)<< std::setw(9)<< std::right<<"RPC pt"
          << "||"<< std::setprecision(2)<< std::setw(9)<< std::right<<"RPC-L1pt"      
          << "||"<< std::setprecision(2)<< std::setw(9)<<std::right<< "L1 bx"
          << "||"<< std::setprecision(2)<< std::setw(9)<< std::right<<"RPC bx"
          << "||"<< std::setprecision(2)<< std::setw(9)<< std::right<<"RPC-L1bx"           
