...
L1MuonMatcherAlgo::match(TrajectoryStateOnSurface & propagated, const std::vector<l1extra::L1MuonParticle> &l1s, float &deltaR, float &deltaPhi) const {
	//std::cout<<"Matcher cc l1s.size() = "<< l1s.size() <<std::endl;
    return matchGeneric(propagated, l1s, preselectionCut_, deltaR, deltaPhi);
}

int
L1MuonMatcherAlgo::match(TrajectoryStateOnSurface & propagated, const std::vector<l1extra::L1MuonParticle> &l1s, float &deltaR, float &deltaPhi,float &deltaEta) const {
	//std::cout<<"Matcher cc l1s.size() = "<< l1s.size() <<std::endl;
    return matchGeneric(propagated, l1s, preselectionCut_, deltaR, deltaPhi, deltaEta);
}
