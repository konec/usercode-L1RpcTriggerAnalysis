...
//
// $Id: L1MuonMatcherAlgo.h,v 1.1 2012/03/09 08:11:50 rybinska Exp $
//
...
/**
  ...
  \version  $Id: L1MuonMatcherAlgo.h,v 1.1 2012/03/09 08:11:50 rybinska Exp $
*/
...
class L1MuonMatcherAlgo {
    public:
        ...
        /// The preselection cut on L1, if specified in the config, is applied before the match
        int match(const reco::Track &tk, const std::vector<l1extra::L1MuonParticle> &l1, float &deltaR, float &deltaPhi, float &deltaEta, TrajectoryStateOnSurface &propagated) const {
            propagated = extrapolate(tk);
            return propagated.isValid() ? match(propagated, l1, deltaR, deltaPhi, deltaEta) : -1;
        }
        ...
        int match(const reco::Candidate &c, const std::vector<l1extra::L1MuonParticle> &l1, float &deltaR, float &deltaPhi, float &deltaEta, TrajectoryStateOnSurface &propagated) const {
            propagated = extrapolate(c);
            return propagated.isValid() ? match(propagated, l1, deltaR, deltaPhi, deltaEta) : -1;
        }
        ...
        int match(TrajectoryStateOnSurface &propagated, const std::vector<l1extra::L1MuonParticle> &l1, float &deltaR, float &deltaPhi, float &deltaEta) const ;
        ...
        template<typename Collection, typename Selector>
        int matchGeneric(TrajectoryStateOnSurface &propagated, const Collection &l1, const Selector &sel, float &deltaR, float &deltaPhi, float &deltaEta) const ;
...
template<typename Collection, typename Selector>
int 
L1MuonMatcherAlgo::matchGeneric(TrajectoryStateOnSurface &propagated, const Collection &l1s, const Selector &sel, float &deltaR, float &deltaPhi, float &deltaEta) const {
  ...
  //  std::cout<<"Matcher l1s.size() = "<< l1s.size() <<std::endl;
